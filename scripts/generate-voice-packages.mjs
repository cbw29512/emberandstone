...
function buildVoiceManifest(topic, draft, narrationText) {
  try {
    ...
    throw new Error("Could not build voice manifest: " + error.message);
  }
}
...
async function processTopic(topic) {
  try {
    ...
    const draftPath = path.join(SCRIPT_OUTPUT_ROOT, topicId, "script-draft.json");
    ...
    const draft = await readJson(draftPath, "script-draft.json for " + topicId);
    validateDraft(draft, topicId);

    const narrationText = draft.narration_script.trim() + "\n";
    const manifest = buildVoiceManifest(topic, draft, narrationText);
